...
2. AE - Arithmetic Expressions
    2.1  Enviroments are mappings: Identifiers(Symbols) -> Values
  ...
  '''
    2.2 fold - Visitor Pattern
    
3. WAE - With-Arithmetic Expressions
    3.1 with Binder - etwas einen Namen geben
  ''' scala
  ...
  '''
    ...
    3.3 Substitution kann verwendet werden um "Namen" in PL zu verstehen
    3.4 Die Korrekte Implementation von Substitution muss mit freien, gebundenen und gebundenen Instanzen von namen und deren Scopes korrekt umgehen können
  ''' scala
...
4. F1- WAE - First order-With-AE    
    ...
    4.1 Warum Funktionen( Prozeduren/Methoden)?
	  * Mit with lässt sich x*x/2 nicht umsetzten (speichern) 
    4.2 First-Order Funktion
	  * no expression
 ...
 '''
    4.5 Static Scope: In einer PL mit "static scope", der Scope eines Identifiers ist eine syntaktisch eingegrenzte Region
	  * Eine typische Region wäre der body einer Funktion oder einem anderen "binding construct" 
 
    4.6 Dynamic Scope: In einer PL mit dynamischen scoping, der Scope eines Identifiers ist das komplette Enviroments der Ausführung des "binding construct"
    
5. FAE Funktionale AE - lambda kalkül
    5.1 FAE ist die Sprache der AE + Funktionsabstraktion und -applikation
    5.2 first class Funktionen
	  * Funktionen werden zu Values und können zurück- oder übergeben werden
    5.3 higher-order Funktionen
	  * Die Funktionen akzeptieren oder returnen andere Funktionen
    5.4 lambda x.x+3 (in Scala) => (x) => x+3
    5.6 desugering: constructive translation
    5.7 Freie Variablen: Um vorzubeuen, das x in einer Methode das "gleiche" x ist wie außerhalb.. Sonst: static scoping!
 ''' scala
...
'''
  5.8 closures werden gebraut, da für die Substitution Parts zur evaluation zurückstellen diese können ohne ihre passendes Env nicht ausgewertet werden
''' scala  
...
6. lcfae - lambda kalkül
    ...
    6.2 call-by-name - -Ähnlich lazy evaluation
 ...
 ''' scala Evalcbn:
  ...
  '''
    ...
    6.4 Thunks (Nicht in Scala umsetzbar)
	  * type Thunk = (Exp, Env)
...
'''
    6.7 Ist es eine gute idee Implicite Mutation PL mit Lazy Evaluation zu mixen?
	  * Nein, da Variablen weiter verwendet werden..
      
7. Haskelzeugs..

8. RCFAE - Recursive Funktionale AE
    8.1 Vorteile von Mutation:
...
9. BCFAE - Boxes and Conditional FAE 
    9.1 Mutable Data Structure: Boxes
''' scala
...
10. Garbage collector
    10.1 Wir setzten es mit einer Mutable Flag um, echte Systeme nutzen Bit Flaggen
    10.2 Mark and Sweep Store 
...
11. Syntatktische- vs. Meta- Interpretation
    11.1 Syntaktische Interpretation
    ...
    11.3 HOAS - Higher-order abstract syntax
    11.4 2 Objekt Sprachen Expressions sind equivalent wenn ihre "denotations" als Meta-Level Expressions sind equivalent im Meta-level
13. CPS Transformation

15. Let/cc - Racket/Scheme
    15.1 speichert die continuation (let/cc k (k 3))
    15.2 nützlich für Multithreading und exceptions
    
16. First class continuation with letcc
    16.1  Für die Umsetzung von continuation in Scala benötigen wir:
...
23. Typ Systeme - (78)
    ...
    23.3 Introduction on Syntax
	  * Relation - Function
	  * Induktion über die Länge
    23.4 Structural Operational Semantics (SOS)
	  * Reduktions Relationen
	  * Digression ?
		* "Computation" Regeln
		* "Congruence" Regeln
    23.5 Grund für Reduktion
	  * Ableitungen
	  * Normalform
...
24. The Lambda Calculus, formal (119)
    ...
    24.11 The Simply Typed Lamdba-Calculus (Lambda_Pfeil/L_P)
	  * Eigenschaften
		* Progress
		* Preservation
	  * Inversion (Umkehrung)
	  ...
	  * Let-bindings
    24.12 Pairs, Tuples and Records
	  * Pairs
	    * Evaluationregeln
	    * Typenregeln
	  * Tupel
...
25. Subtyping - (85)
    25.1 Subtype Relation
	  * Records
	  * Arrow types
	  * Top
	  * General Rules
    25.2 Eigenschaften von Subtyping
	  * Safety
	  * Preservation
	  * Inversion
    25.3 Subtyping mit anderen Features
	* Ascription
	* Casting
	...
	* Subtyping und Arrays
    25.4 Algorithmic Subtyping
	* Syntax-directed Regeln
	* Non-Syntax-directedness of 
		* Typing
		* Subtyping
	* Entwicklung eines Alorithmus für die "subtyping relation"
    25.5 Algorithmic Typing
	  * Type Checker for the lambda-calculus with subtyping
	  * Typing Rules
    25.6 Meets and Joins
	  * Meets: 
	  * Joins:
    25.7 Universal Types
	  * System F
	  * Eigenschaften von System F
	
26. Typ Rekonstruktion - (19)
    26.1 aka Type inference
    26.2 Vor- und Nachteile
	  + Type checker ist einfach
	  - Typ Annotationen immer hinzuschreiben kann ermüdend sein, da viele Annotationen eindeutig sind
    26.3 Robinson Algorithm
    26.4 Curry-Howard Isomorphism aka "Propositions as Types" - Zeichen der Logik
    26.5 Intuitionistische Logik
    26.6 Reduktion = Beweis Normalisation
    26.7 CPS Transformation = Doppelte Negation
	
27. Modeling Java - (71)
    27.1 Featherweight Java (FJ)
    27.2 Formalizing FJ
    27.3 JF Syntax
    27.4 Subtyping
    27.5 Evaluation 
    ...
    27.8 More on Evaluation Context 

29. Existential Types - (13)
    29.1 Building and using terms with existential types
	  * Introduction and Elimination Regeln
    29.2 Encoding existential types by universal types
	  * SML
    29.3 Open vs. closed Scope

30. Higher-Order Typen - (22)
    30.1 Higher-Order Types in Haskell
    30.2 Evaluation
    30.3 Kinding rules
    30.4 Typing rules
    30.5 Type Equivalence
    30.6 Erweiterung von System F
    30.7 Higher-Order Existentials
    30.8 Algorithmic Type-Checking for F
